...
function evalKey(e: { suite: string; ran_at?: string }): string {
  return `${e.suite}@${e.ran_at ?? "unknown"}`;
}

function arrEquals(a: string[] | undefined, b: string[] | undefined): boolean {
  const sa = [...(a ?? [])].sort();
  const sb = [...(b ?? [])].sort();
...
export function diffProvenance(previous: ProvenanceDoc, next: ProvenanceDoc, _opts: DiffOptions = {}): ProvenanceDiff {
  ...
  if (previous.prompt.id !== next.prompt.id) push("prompt-id-changed", `${previous.prompt.id} → ${next.prompt.id}`);
  ...
  if (!prevParent && nextParent) push("lineage-parent-added", nextParent);
  if (prevParent && !nextParent) push("lineage-parent-removed", prevParent);
  ...
  if (prevState !== nextState) {
    ...
    const terminal: ApprovalState[] = ["deprecated", "revoked"];
    if (nextRank < prevRank || terminal.includes(nextState)) push("approval-state-regressed", detail);
    else push("approval-state-advanced", detail);
  }
  if ((previous.approval.policy_uri ?? "") !== (next.approval.policy_uri ?? "")) push("approval-policy-changed");
  ...
  for (const r of nextR) if (!prevR.has(r)) push("authorship-reviewer-added", r);
  for (const r of prevR) if (!nextR.has(r)) push("authorship-reviewer-removed", r);

  // ─── intent ────────────────────────────────────────────────────────────
  if ((previous.intent?.purpose ?? "") !== (next.intent?.purpose ?? "")) push("intent-purpose-changed");
  if (!arrEquals(previous.intent?.in_scope, next.intent?.in_scope)) push("intent-in-scope-changed");
  if (!arrEquals(previous.intent?.out_of_scope, next.intent?.out_of_scope)) push("intent-out-of-scope-changed");
  if (!arrEquals(previous.intent?.models_supported, next.intent?.models_supported)) push("intent-models-supported-changed");

  // ─── evaluations ───────────────────────────────────────────────────────
  const prevEvals = new Map<string, { suite: string; passed?: boolean; score?: number; ran_at?: string }>();
  for (const e of previous.evaluations ?? []) prevEvals.set(evalKey(e), e);
  const nextEvals = new Map<string, { suite: string; passed?: boolean; score?: number; ran_at?: string }>();
  for (const e of next.evaluations ?? []) nextEvals.set(evalKey(e), e);

  const evalAdded: string[] = [];
  const evalRemoved: string[] = [];
...
function assertDoc(doc: ProvenanceDoc, side: string): void {
  if (!doc || typeof doc !== "object") throw new Error(`${side} must be a ProvenanceDoc object`);
  if (!doc.prompt) throw new Error(`${side}.prompt is required`);
  if (!doc.approval) throw new Error(`${side}.approval is required`);
  if (!doc.authorship) throw new Error(`${side}.authorship is required`);
}
